...
impl ClientState {
    ...
    async fn initialize(&mut self) {
        ...
        if self.use_load_balancer {
            logging::info!("Client will use load balancer for indexer access");
        } else if let Some(idx) = &self.direct_indexer_address {
            logging::info!("Client will use direct indexer access at {}", idx.to_string());
        } else {
            logging::warn!("No indexer access configured");
        }
    }
    ...
    async fn toggle_load_balancer_usage(&mut self) -> bool {
        ...
        logging::info!("Load balancer usage set to: {}", self.use_load_balancer);
        true
    }
    ...
    #[remote]
    #[local]
    async fn lb_add_to_index(&mut self, index_name: String, item: String) -> IndexResult<bool> {
    ...
    #[remote]
    #[local]
    async fn lb_remove_from_index(&mut self, index_name: String, item: String) -> IndexResult<bool> {
        self.request_count += 1;
    ...
    #[remote]
    #[local]
    async fn lb_get_index(&mut self, index_name: String) -> IndexResult<Option<Vec<String>>> {
        self.request_count += 1;
    ...
    #[remote]
    #[local]
    async fn lb_list_indices(&mut self) -> IndexResult<Vec<String>> {
        self.request_count += 1;
