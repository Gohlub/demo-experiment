...
impl IndexerState {
    ...
    async fn initialize(&mut self) {
        logging::info!("Indexer initializing");
        ...
        if let Some(lb_addr) = &self.load_balancer_address {
            ...
            logging::info!("No load balancer configured, running independently");
        }
        ...
        if let Some(lb_addr) = lb_address {
    ...
    async fn register_with_load_balancer(&mut self, lb_address: String) -> bool {
        ...
        match register_indexer_remote_rpc(&lb_address, my_address, self.max_capacity).await {
